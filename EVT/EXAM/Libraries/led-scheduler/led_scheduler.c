...
static void LED_update_state(led_state_t *state, const led_itf_t *itf)
{
    ...
    uint8_t pattern_size = state->pattern->size;
    ...
    if(state->repeat_count)
    {
        ...
        uint16_t delay = state->pattern->delay_list[state->pattern_idx];
    ...
    switch(event->type)
    {
        case LED_EVENT_NEW_STATE:
        {
            ...
            if(led_num < num_of_leds)
            {
                ...
                uint16_t delay = led_state->pattern->delay_list[0];
